  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      if (tipoOperacion.respuesta == "inscripcion_realizada") {
        notificacionExito(
          "OPERACIÓN EXITOSA",
          `¡Te has inscripto correctamente! El administrador del sitio será notificado.`
        );
      }
      //Operaciones erroneas
        ...
        notificacionError(
          ...
          `Lo sentimos, no te encuentras registrado en nuestro ranking.`
        );
        setTimeout(() => {
          location.reload();
        }, 5000);
        ...
        notificacionError(
          "HA OCURRIDO UN ERROR",
          `La operación ha fracasado, esto puede ocurrir en caso de que ya estes inscripto a este torneo.`
        );
      } else if (tipoOperacion.respuesta == "club_fallido") {
        notificacionError(
          "HA OCURRIDO UN ERROR",
          `La operación ha fracasado, esto puede ocurrir en caso de que este participante ya este inscripto a este torneo.`
        );
      }
    },
  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      if (tipoOperacion.email != "inscripcion-torneo") {
        notificacionExito(
          "OPERACIÓN EXITOSA",
          "Has realizado correctamente la solicitud y llegará a manos de un administrador en breve"
        );
      }
    },
    error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function leerFormulario(formulario, accion) {
  if (formulario == "inscripcion") {
    ...
    /* Telefono */
    if (!validarCampo(telefonoClub, 0, 9999, formulario_cTelefono))
      return false;
...
$(document).ready(function () {
  ...
  $(botonPesosM).on("click", function () {
    ...
    let pesoAnterior = $(
      `#pesos tr:nth-child(${positionInTable - 1}) td:nth-child(1)`
    ).html();
    ...
    for (let i = 0; i < competidoresRanking.length; i++) {
      ...
      if (
        generoActual != genero ||
        (signo == "positivo" && peso > pesoActual) ||
        (signo == "negativo" &&
          (pesoAnterior >= pesoActual || peso < pesoActual))
      ) {
        $(competidor).hide();
